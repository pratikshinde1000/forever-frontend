...
const PlaceOrder = () => {
  ...
  console.log("razorpay", { error, isLoading })
  ...
  const razorpay_init = (razorpay_data) => {
    console.log('raz', razorpay_data);
    const options = {
      key: import.meta.VITE_RAZORPAY_KEY_ID,
      amount: razorpay_data.amount,
      currency: razorpay_data.currency,
      name: `Order Payment for ${razorpay_data?.receipt}`,
      order_id: razorpay_data?.id,
      receipt: razorpay_data.receipt,
      handler: async (response) => {
        console.log(response);
        try {
          const { data } = await postRequest(`${backendUrl}/api/order/verify/razorpay`, response, { headers: { token } })
          if (data.success) {
          ...
          toast.error(error?.response?.data?.error || 'Failed to process the payment');
        }
      }
    }
  ...
  const handleSubmit = async (event) => {
    try {
      ...
      const response = await postRequest(`${backendUrl}/api/order/create`, data, { headers: { token } });
      console.log('response', response?.data);
      if (response?.data) {

        if (paymentMethod === 'RAZORPAY' && response?.data?.razorpay_data) {
          razorpay_init(response?.data?.razorpay_data);
        } else {
  ...
  return (
    <>
      {!token ? <Login /> : <form onSubmit={handleSubmit} className='flex flex-col sm:flex-row justify-between gap-4 pt-5 sm:pt-14 min-h-[80vh] border-t'>
        {/* Left Side */}
        ...
        <div className='mt-8'>
          ...
          <div className='mt-12'>
            ...
            <div className='w-full text-end mt-8'>
              {(submitLoading && isLoading) && <p>Loading Razorpay...</p>}
              {(submitLoading && error) && <p>Error loading Razorpay: {error}</p>}
              <button type='submit' disabled={submitLoading && isLoading} className='bg-black text-white px-16 py-3 text-sm disabled:bg-gray-700 '>PLACE ORDER</button>
            </div>
